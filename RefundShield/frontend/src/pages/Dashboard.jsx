...
import { ShieldAlert, Activity, ShoppingCart, IndianRupee } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mt-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-gray-800">Recent High-Risk Orders</h2>
            ...
            <tbody className="divide-y divide-gray-50">
              {orders.slice(0, 10).map((order) => (
                <tr key={order.order_id} className="hover:bg-gray-50 transition-colors">
                  <td className="py-4 text-sm font-medium text-gray-900">{order.order_id}</td>
                  <td className="py-4 text-sm text-gray-600">{order.account_id}</td>
                  <td className="py-4 text-sm font-medium">₹{order.amount.toFixed(2)}</td>
                  <td className="py-4 text-sm">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${order.refund_requested ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-700'}`}>
